...
const checkComment = (type: number) => {
  ...
  (mint.filter(replyModule.content, { replace: false }) as any).then((data: any) => {
    if (!data.pass) {
      ElNotification({
        title: '评论包含敏感词汇',
